...
export default function Page() {
  ...
  const humidityTone: Tone = useMemo(() => {
    if (sensor.kelembaban === null) return "normal";
    if (sensor.kelembaban > 75) return "danger";
    if (sensor.kelembaban >= 60) return "warning";
    return "normal";
  }, [sensor.kelembaban]);

  const ldrTone: Tone = useMemo(() => {
    if (sensor.ldr === null) return "normal";
    if (sensor.ldr === "Gelap") return "warning";
    return "normal";
  }, [sensor.ldr]);

  const toneStyles: Record<Tone, { color: string; card: string; accent: string }> = {
    normal: {
      color: "bg-emerald-500",
      card: "border-emerald-200/80 bg-emerald-50/80 dark:border-emerald-500/20 dark:bg-emerald-500/10",
    ...
    danger: {
      color: "bg-rose-500",
      card: "border-rose-200/80 bg-rose-50/80 dark:border-rose-500/20 dark:bg-rose-500/10",
      accent: "from-rose-500 via-red-500 to-orange-500"
    }
  };

  const humidityCardStyles: Record<Tone, string> = {
    normal: "border-cyan-200/80 bg-cyan-50/80 dark:border-cyan-500/20 dark:bg-cyan-500/10",
    warning: "border-lime-200/80 bg-lime-50/80 dark:border-lime-500/20 dark:bg-lime-500/10",
    danger: "border-fuchsia-200/80 bg-fuchsia-50/80 dark:border-fuchsia-500/20 dark:bg-fuchsia-500/10"
  };

  const ldrCardStyles: Record<Tone, string> = {
    normal: "border-amber-200/80 bg-amber-50/80 dark:border-amber-500/20 dark:bg-amber-500/10",
    warning: "border-indigo-200/80 bg-indigo-50/80 dark:border-indigo-500/20 dark:bg-indigo-500/10",
    danger: "border-indigo-200/80 bg-indigo-50/80 dark:border-indigo-500/20 dark:bg-indigo-500/10"
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6 animate-fade-in">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <SensorCard
            ...
            icon={Thermometer}
            color={toneStyles[tempTone].color}
            cardClassName={toneStyles[tempTone].card}
            accentClassName={toneStyles[tempTone].accent}
          ...
          <SensorCard
            ...
            icon={Droplets}
            color="bg-sky-500"
            cardClassName={humidityCardStyles[humidityTone]}
            accentClassName="from-sky-400 via-cyan-400 to-blue-400"
            isLoading={sensor.kelembaban === null}
            subtitle={
              sensor.kelembaban === null
                ? "Menunggu data kelembaban"
                : sensor.kelembaban > 75
                ? "Status: Sangat Lembap"
                : sensor.kelembaban >= 60
                ? "Status: Lembap"
                : "Status: Normal"
            }
          />
          <SensorCard
            ...
            cardClassName={ldrCardStyles[ldrTone]}
            ...
            subtitle={sensor.ldr === null ? "Menunggu data LDR" : `Status: ${sensor.ldr}`}
          />
        </section>
