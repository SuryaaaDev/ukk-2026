...
export default function TemperatureChart({ data }: TemperatureChartProps) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-md transition dark:border-white/10 dark:bg-slate-900/80">
      <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">Chart Suhu</h2>
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 10, left: -15, bottom: 0 }}>
            ...
            <Tooltip />
            <Line isAnimationActive type="monotone" dataKey="suhu" stroke="#f97316" strokeWidth={2.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </section>
